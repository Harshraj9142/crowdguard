    ...
    try {
        ...
        if (lat && lon) {
            try {
                ...
                locationName = geoData.address?.city || geoData.address?.town || geoData.address?.village || geoData.address?.county || 'Local';
            } catch (e) {
...
app.get('/api/nearby-responders', async (req, res) => {
    try {
        ...
        const responders = data.elements.map((element: any) => {
